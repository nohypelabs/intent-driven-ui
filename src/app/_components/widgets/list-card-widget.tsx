    ...
    <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="divide-y divide-slate-800/40">
        {visibleItems.map((item, idx) => (
          <div key={idx} className="px-5 py-3.5 hover:bg-slate-800/40 transition-colors duration-150 cursor-default">
            ...
            <p className="text-xs text-slate-500 mt-0.5 ml-3.5">{item.value}</p>
          </div>
        ))}
      </div>

      {hiddenCount > 0 && (
        <div className="px-5 py-2.5 border-t border-slate-800/60 bg-slate-900/50 text-center">
