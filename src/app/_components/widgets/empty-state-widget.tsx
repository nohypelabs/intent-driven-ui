...
export function EmptyStateWidget({ message, suggestions, onSuggestionClick }: EmptyStateProps) {
  return (
    <div className="w-full max-w-md p-6 bg-slate-900/80 border border-slate-800 rounded-2xl">
          ...
          <div className="flex flex-wrap gap-1.5">
            {suggestions.map((s, idx) => (
              <button
                ...
                className="text-[11px] px-3 py-1.5 bg-slate-800/80 hover:bg-slate-800 text-slate-300 hover:text-white rounded-full border border-slate-700 hover:border-indigo-500/50 transition-all duration-200 text-left"
              >
                {s}
              </button>
            ))}
