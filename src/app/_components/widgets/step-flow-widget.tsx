...
export function StepFlowWidget(props: StepFlowProps) {
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex gap-3 relative">
                {/* Vertical Line */}
                {!isLast && (
                  <div className="absolute left-[11px] top-6 w-0.5 h-full bg-slate-800" />
                )}
                ...
                <div className={`flex-1 pb-5 ${isLast ? 'pb-0' : ''}`}>
                  <div className="flex items-center gap-2">
                    ...
                    {step.status === 'current' && (
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 animate-pulse">
                        ACTIVE
                      </span>
                    )}
                  </div>
